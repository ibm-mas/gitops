...
Each set of YAML values is used to render the **Cluster Root Application Set** template (i.e. a **Cluster Root Application**, which itself renders the ibm-mas-cluster-root Helm Chart):
```yaml
  template:
    metadata:
...
The Cluster Root Application Helm Chart contains templates to conditionally render ArgoCD Applications one their associated configuration appears in the Helm values generated by the **Cluster Root Application Set**. For example, the [000-ibm-operator-catlog-app template](root-applications/ibm-mas-cluster-root/templates/000-ibm-operator-catalog-app.yaml) is guarded by:
```yaml
{{- if not (empty .Values.ibm_operator_catalog) }}
```
When the `ibm_operator_catalog` key appears, the template will be rendered. This will result in an ArgoCD Application that will render the [ibm-operator-catalog Helm Chart](cluster-applications/000-ibm-operator-catalog) into the target cluster.

Because we know some of the values will be [inline-path placeholders](https://argocd-vault-plugin.readthedocs.io/en/stable/howitworks/#inline-path-placeholders) for referencing secrets in the **Secrets Vault**, we use the AVP plugin source to render the Helm chart, building the value structure expected by the chart as YAML:
```yaml
kind: Application
metadata:
...
As we need the templates from the [ibm-operator-catalog Helm Chart](cluster-applications/000-ibm-operator-catalog) to be rendered into the targer cluster, we specify:
```yaml
  destination:
...
- The Git File generators look for a different set of named YAML files at the **instance** level in the **Config Git Repo**:
  ...
  ```
- the **Instance Root Applications** that it generates source the [ibm-mas-instance-root Helm Chart](root-applications/ibm-mas-instance-root).
