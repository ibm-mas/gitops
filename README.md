...
The Cluster Root Application Helm Chart contains templates to conditionally render ArgoCD Applications as their associated configuration appears in the Helm values generated by the **Cluster Root Application Set**. For example, the [000-ibm-operator-catlog-app template](root-applications/ibm-mas-cluster-root/templates/000-ibm-operator-catalog-app.yaml) is guarded by:
```yaml
{{- if not (empty .Values.ibm_operator_catalog) }}
```

When the `ibm_operator_catalog` key appears, 

... TODO

We want the templates to be rendered into the targer cluster, so we specify:
```yaml
  destination:
