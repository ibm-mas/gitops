...
Contains a job that runs last (`05-postsync-setup-db2_Job.yaml`). This registers the `${ACCOUNT_ID}<sep>${CLUSTER_ID}<sep>${MAS_INSTANCE_ID}<sep>jdbc<sep>${DB2_INSTANCE_NAME}<sep>config` secret in the **Secrets Vault** used to share some information that is generated at runtime with other ArgoCD Applications. When the backend is AWS, `<sep>` is `/`. This job also performs some special configuration steps that are required if the Db2u database is intended for use by the Manage MAS Application.
