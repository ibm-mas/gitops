...
**The Kustomize-based RBAC in this directory is only required when using the [pre-install repository](https://github.com/ibm-mas/pre-install) for cluster prerequisites.**

If you are using `cluster_admin_role=true` in a separate ArgoCD instance to deploy cluster-level resources, the [`600-application-admin-rbac`](https://github.com/ibm-mas/gitops/tree/main/instance-applications/600-application-admin-rbac/) Helm chart will **automatically create all necessary RBAC** for the `application_admin_role`. In this scenario, you do not need to use the Kustomize configurations in this directory.
...
The [`kustomize/`](https://github.com/ibm-mas/gitops/tree/main/rbac/kustomize/) directory provides a declarative, GitOps-friendly approach using Kustomize for managing namespace-scoped RBAC **when using the pre-install repository**.
...
See [`kustomize/README.md`](https://github.com/ibm-mas/gitops/tree/main/rbac/kustomize/README.md) for detailed usage and examples.

## Files and Folders

- [`exmaple/build-ouptut`](https://github.com/ibm-mas/gitops/tree/main/rbac/exmaple/build-ouptut) - Example output of the `kustomize build rbac/kustomize/overlays/mas-argocd-argocd-application-controller` command
- [`exmaple/overlays`](https://github.com/ibm-mas/gitops/tree/main/rbac/exmaple/overlays) - Example of an overlay that would be generated by the `generate_rbac_overlays.py` script. This would be placed in the `overlays` folder under [`kustomize/`](https://github.com/ibm-mas/gitops/tree/main/rbac/kustomize/) directory normally.
- [`kustomize/`](https://github.com/ibm-mas/gitops/tree/main/rbac/kustomize/) - Kustomize-based RBAC management (base, components, overlays)
- [`generate_rbac_overlays.py`](https://github.com/ibm-mas/gitops/tree/main/rbac/generate_rbac_overlays.py) - Generator script for creating namespaced overlay directories under [`kustomize/`](https://github.com/ibm-mas/gitops/tree/main/rbac/kustomize/)
...
See [`kustomize/README.md`](https://github.com/ibm-mas/gitops/tree/main/rbac/kustomize/README.md) for creating custom overlays and adding namespaces.

## generate_rbac_overlays.py

[`generate_rbac_overlays.py`](https://github.com/ibm-mas/gitops/tree/main/rbac/generate_rbac_overlays.py) generates the Kustomize overlay directories for one or more MAS instances. Because Kustomize has no native loop/iteration, a separate subdirectory is required per namespace; this script automates that process.
...
**When using this approach, the [`600-application-admin-rbac`](https://github.com/ibm-mas/gitops/tree/main/instance-applications/600-application-admin-rbac/) Helm chart automatically creates all necessary RBAC for the `application_admin_role`.** You do **not** need to use the Kustomize configurations in this directory.
...
**When using the pre-install repository, you MUST use the Kustomize-based RBAC configurations in this directory** to create the necessary namespace-scoped RBAC for the `application_admin_role`. Please see the [pre-install](https://github.com/ibm-mas/pre-install) repo for more details.
...
- **Solution**: Add a namespace overlay for X in `rbac/kustomize/overlays/<service-account-name>/` via the [`generate_rbac_overlays.py`](https://github.com/ibm-mas/gitops/tree/main/rbac/generate_rbac_overlays.py) script and re-apply
...
- [MAS GitOps Repository Structure](https://github.com/ibm-mas/gitops/tree/main/README.md)
