...
If you are using `cluster_admin_role=true` in a separate ArgoCD instance to deploy cluster-level resources, the [`600-application-admin-rbac`](../instance-applications/600-application-admin-rbac/) Helm chart will **automatically create all necessary RBAC** for the `application_admin_role`. In this scenario, you do not need to use the Kustomize configurations in this directory.
...
The [`kustomize/`](kustomize/) directory provides a declarative, GitOps-friendly approach using Kustomize for managing namespace-scoped RBAC **when using the pre-install repository**.
...
- [`exmaple/overlays`](exmaple/overlays) - Example of an overlay that would be generated by the `generate_rbac_overlays.py` script. This would be placed in the `overlays` folder under [kustomize/`](kustomize/) directory normally.
- [`kustomize/`](kustomize/) - Kustomize-based RBAC management (base, components, overlays)
- [`generate_rbac_overlays.py`](generate_rbac_overlays.py) - Generator script for creating namespaced overlay directories under [kustomize/`](kustomize/)
...
[`generate_rbac_overlays.py`](generate_rbac_overlays.py) generates the Kustomize overlay directories for one or more MAS instances. Because Kustomize has no native loop/iteration, a separate subdirectory is required per namespace; this script automates that process.
...
**When using this approach, the [`600-application-admin-rbac`](../instance-applications/600-application-admin-rbac/) Helm chart automatically creates all necessary RBAC for the `application_admin_role`.** You do **not** need to use the Kustomize configurations in this directory.
...
- **Solution**: Add a namespace overlay for X in `rbac/kustomize/overlays/<service-account-name>/` via the [`generate_rbac_overlays.py`](generate_rbac_overlays.py) script and re-apply
